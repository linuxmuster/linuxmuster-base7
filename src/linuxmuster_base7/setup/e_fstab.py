...
def mask_redundant_quotaon_units():
    """Mask systemd's quotaon-root.service/quotaon.service/quotaon@.service.

    quotaon-root.service/quotaon.service: once the ext4 on-disk quota
    feature is active, the kernel enables quota tracking automatically as
    soon as the filesystem is mounted (verified with repquota showing real
    usage without these units ever having run successfully). Calling
    quotaon afterwards to turn on what's already on fails ("quotaon: using
    . on <dev> [<mnt>]: File exists"), permanently failing the unit at
    every boot for no functional benefit.

    quotaon@.service: systemd's own per-mount template unit (from the
    quota package, /usr/lib/systemd/system/quotaon@.service - not
    generated by us), instantiated once per non-root mount with
    usrquota/grpquota options (e.g. quotaon@srv-linbo.service). Every
    non-root filesystem here uses the external-quota-file mechanism (see
    the non-root branch in main()), so its aquota files only exist once
    e_fstab.py itself has run quotacheck - at boot, before that, this unit
    fails with "cannot find ... aquota.user/aquota.group". Masking the
    *template* (with the trailing "@") masks every instance in one call,
    confirmed live - a masked template unit refuses to resolve any
    instance name at all, rather than needing one mask call per mount.

    Masking a unit name systemd hasn't generated (yet) is a harmless no-op.
    """
    subprocess.run(['systemctl', 'mask', 'quotaon-root.service', 'quotaon.service', 'quotaon@.service'],
...
def main():
    ...
    ext4_mounts = []
    enable_quota = False
    # whether at least one ext4 mount doesn't genuinely have quota turned on
    # yet (see is_quota_on() - "quota" in the active mount options is not
    # enough, a fresh mount from a corrected fstab shows that immediately
    # even though quotaon was never called). This must be checked
    ...
    if mounts_ready_for_activation:
        ...
        # quota - confirmed live that it silently never creates
        # aquota.group otherwise, only aquota.user.
        # One mountpoint per invocation - confirmed live that quotacheck
        # silently only scans the *first* of several mountpoint arguments
        # and ignores the rest (no error, exit 0), unlike quotaon/quotaoff
        # which do handle multiple arguments correctly.
